...
// Package git provides tool for parsing URL to git repo spec
package git
...
func NewRepoSpecFromURL(n string) (*RepoSpec, error) {
	...
	return &RepoSpec{
		...
		Path: path, Ref: gitRef, GitSuffix: suffix,
...
func peelQuery(arg string) (path, ref string, duration time.Duration, submodules bool) {
...
func parseHostSpec(n string) (host, spec string) {
	// Start accumulating the host part.
	for _, p := range []string{
		// Order matters here.
		"git::", "gh:", "ssh://", "https://", "http://",
		"git@", "github.com:", "github.com/"} {
		if len(p) < len(n) && strings.EqualFold(n[:len(p)], p) {
			n = n[len(p):]
			host += p
		}
	}
