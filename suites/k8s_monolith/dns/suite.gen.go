...
func (s *Suite) SetupSuite() {
	...
	r.Run(`ipdock=$(docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' cmd-nse-simple-vl3-docker)` + "\n" + `echo Selected dockerIP: $ipdock` + "\n" + `[[ ! -z $ipdock ]]`)
	...
	r.Run(`docker exec -d -i cmd-nse-simple-vl3-docker cp /etc/resolv.conf /etc/resolv_init.conf`)
	r.Run(`docker exec -d -i cmd-nse-simple-vl3-docker sh -c "echo 'nameserver 127.0.1.1' > /etc/resolv.conf"`)
	...
	r.Run(`docker cp coredns-config cmd-nse-simple-vl3-docker:/`)
	r.Run(`docker cp dnsentries.db cmd-nse-simple-vl3-docker:/`)
	r.Run(`docker exec -d cmd-nse-simple-vl3-docker coredns -conf coredns-config`)
}
func (s *Suite) Test() {}
