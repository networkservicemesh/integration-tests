...
func (s *Suite) TestSriovKernel2Noop() {
	...
	r.Run(`NSC=$(kubectl -n ${NAMESPACE} get pods -l app=nsc --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')`)
...
func (s *Suite) TestVfio2Noop() {
	...
	s.T().Cleanup(func() {
		r.Run(`NSE=$(kubectl -n ${NAMESPACE} get pods -l app=nse --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')`)
	...
	r.Run(`kubectl -n ${NAMESPACE} wait --for=condition=ready --timeout=1m pod -l app=nse`)
	r.Run(`NSC=$(kubectl -n ${NAMESPACE} get pods -l app=nsc --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')`)
	r.Run(`kubectl -n ${NAMESPACE} logs ${NSC} sidecar |` + "\n" + `  grep "All client init operations are done."`)
	r.Run(`PING_RESULTS=$(kubectl -n ${NAMESPACE} exec ${NSC} --container pinger -- /bin/bash -c ' \` + "\n" + `  /root/dpdk-pingpong/build/app/pingpong                                                \` + "\n" + `    --no-huge                                                                           \` + "\n" + `    --                                                                                  \` + "\n" + `    -n 500                                                                              \` + "\n" + `    -c                                                                                  \` + "\n" + `    -C 0a:11:22:33:44:55                                                                \` + "\n" + `    -S 0a:55:44:33:22:11                                                                \` + "\n" + `' 2>&1) || (echo "${PING_RESULTS}" 1>&2 && false)`)
	r.Run(`PONG_PACKETS="$(echo "${PING_RESULTS}" | grep "rx .* pong packets" | sed -E 's/rx ([0-9]*) pong packets/\1/g')" \` + "\n" + `  || (echo "${PING_RESULTS}" 1>&2 && false)`)
