...
func (s *Suite) TestDns() {
	...
	r.Run(`NAMESPACE=($(kubectl create -f https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/4f418e7f59e97c126e356ac2b44d39596e969fd0/examples/features/namespace.yaml)[0])` + "\n" + `NAMESPACE=${NAMESPACE:10}`)
...
func (s *Suite) TestNse_composition() {
	...
	r.Run(`kubectl create -f https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/4f418e7f59e97c126e356ac2b44d39596e969fd0/examples/features/nse-composition/nse-composition-ns.yaml`)
	r.Run(`kubectl apply -k .`)
	r.Run(`kubectl wait --for=condition=ready --timeout=1m pod -l app=nsc-kernel -n ${NAMESPACE}`)
	r.Run(`kubectl wait --for=condition=ready --timeout=1m pod -l app=nse-kernel -n ${NAMESPACE}`)
	r.Run(`NSC=$(kubectl get pods -l app=nsc-kernel -n ${NAMESPACE} --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')`)
...
func (s *Suite) TestScale_from_zero() {
	...
	r.Run(`kubectl apply -f https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/4f418e7f59e97c126e356ac2b44d39596e969fd0/examples/features/scale-from-zero/autoscale-netsvc.yaml`)
	r.Run(`kubectl apply -k .`)
