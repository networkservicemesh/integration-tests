...
func (s *Suite) SetupSuite() {
	...
	r.Run(`kubectl apply -k https://github.com/networkservicemesh/deployments-k8s/examples/ovs?ref=b46cd28bf2c57d9cf4660c580d76f4cfd3fd422a`)
...
func (s *Suite) TestWebhook_smartvf() {
	...
	r.Run(`WH=$(kubectl get pods -l app=admission-webhook-k8s -n nsm-system --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')` + "\n" + `kubectl wait --for=condition=ready --timeout=1m pod ${WH} -n nsm-system`)
	r.Run(`kubectl apply -k https://github.com/networkservicemesh/deployments-k8s/examples/features/webhook-smartvf?ref=b46cd28bf2c57d9cf4660c580d76f4cfd3fd422a`)
...
func (s *Suite) TestKernel2KernelVLAN() {
	...
	r.Run(`kubectl apply -k https://github.com/networkservicemesh/deployments-k8s/examples/use-cases/Kernel2KernelVLAN?ref=b46cd28bf2c57d9cf4660c580d76f4cfd3fd422a`)
...
func (s *Suite) TestSmartVF2SmartVF() {
	...
	r.Run(`kubectl apply -k https://github.com/networkservicemesh/deployments-k8s/examples/use-cases/SmartVF2SmartVF?ref=b46cd28bf2c57d9cf4660c580d76f4cfd3fd422a`)
