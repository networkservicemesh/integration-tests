...
func (s *Suite) SetupSuite() {
	...
	r.Run(`kubectl apply -k https://github.com/networkservicemesh/deployments-k8s/examples/ovs?ref=6bc76cb7911f23f4ad5e9fef9060f8d0c9bbe37b`)
...
func (s *Suite) TestWebhook_smartvf() {
	...
	r.Run(`WH=$(kubectl get pods -l app=admission-webhook-k8s -n nsm-system --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')` + "\n" + `kubectl wait --for=condition=ready --timeout=1m pod ${WH} -n nsm-system`)
	r.Run(`kubectl apply -k https://github.com/networkservicemesh/deployments-k8s/examples/features/webhook-smartvf?ref=6bc76cb7911f23f4ad5e9fef9060f8d0c9bbe37b`)
...
func (s *Suite) TestKernel2kernelvlan() {
	...
	r.Run(`kubectl apply -k https://github.com/networkservicemesh/deployments-k8s/examples/use-cases/Kernel2KernelVLAN?ref=6bc76cb7911f23f4ad5e9fef9060f8d0c9bbe37b`)
...
func (s *Suite) TestSmartvf2smartvf() {
	...
	r.Run(`kubectl apply -k https://github.com/networkservicemesh/deployments-k8s/examples/use-cases/SmartVF2SmartVF?ref=6bc76cb7911f23f4ad5e9fef9060f8d0c9bbe37b`)
