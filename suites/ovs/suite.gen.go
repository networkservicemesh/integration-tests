...
func (s *Suite) SetupSuite() {
	...
	r.Run(`kubectl apply -k https://github.com/networkservicemesh/deployments-k8s/examples/ovs?ref=a38b2a658dfbfa74add29800850a418b670f99ac`)
...
func (s *Suite) TestWebhook_smartvf() {
	...
	r.Run(`WH=$(kubectl get pods -l app=admission-webhook-k8s -n nsm-system --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')` + "\n" + `kubectl wait --for=condition=ready --timeout=1m pod ${WH} -n nsm-system`)
	r.Run(`kubectl apply -k https://github.com/networkservicemesh/deployments-k8s/examples/features/webhook-smartvf?ref=a38b2a658dfbfa74add29800850a418b670f99ac`)
...
func (s *Suite) TestKernel2KernelVLAN() {
	...
	r.Run(`kubectl apply -k https://github.com/networkservicemesh/deployments-k8s/examples/use-cases/Kernel2KernelVLAN?ref=a38b2a658dfbfa74add29800850a418b670f99ac`)
...
func (s *Suite) TestSmartVF2SmartVF() {
	...
	r.Run(`kubectl apply -k https://github.com/networkservicemesh/deployments-k8s/examples/use-cases/SmartVF2SmartVF?ref=a38b2a658dfbfa74add29800850a418b670f99ac`)
