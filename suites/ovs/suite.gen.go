...
func (s *Suite) SetupSuite() {
	...
	r.Run(`kubectl apply -k https://github.com/networkservicemesh/deployments-k8s/examples/ovs?ref=31c56bcc4dbebc86a63d5fcdeff9b0e516f1dfde`)
...
func (s *Suite) TestWebhook_smartvf() {
	...
	r.Run(`WH=$(kubectl get pods -l app=admission-webhook-k8s -n nsm-system --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')` + "\n" + `kubectl wait --for=condition=ready --timeout=1m pod ${WH} -n nsm-system`)
	r.Run(`kubectl apply -k https://github.com/networkservicemesh/deployments-k8s/examples/features/webhook-smartvf?ref=31c56bcc4dbebc86a63d5fcdeff9b0e516f1dfde`)
...
func (s *Suite) TestKernel2KernelVLAN() {
	...
	r.Run(`kubectl apply -k https://github.com/networkservicemesh/deployments-k8s/examples/use-cases/Kernel2KernelVLAN?ref=31c56bcc4dbebc86a63d5fcdeff9b0e516f1dfde`)
...
func (s *Suite) TestSmartVF2SmartVF() {
	...
	r.Run(`kubectl apply -k https://github.com/networkservicemesh/deployments-k8s/examples/use-cases/SmartVF2SmartVF?ref=31c56bcc4dbebc86a63d5fcdeff9b0e516f1dfde`)
