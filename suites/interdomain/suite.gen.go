...
func (s *Suite) SetupSuite() {
	...
	r.Run(`cat > patch-nsmgr-proxy.yaml <<EOF` + "\n" + `---` + "\n" + `apiVersion: apps/v1` + "\n" + `kind: Deployment` + "\n" + `metadata:` + "\n" + `  name: nsmgr-proxy` + "\n" + `spec:` + "\n" + `  template:` + "\n" + `    metadata:` + "\n" + `      annotations:` + "\n" + `        spiffe.io/federatesWith: nsm.cluster2,nsm.cluster3` + "\n" + `EOF`)
	r.Run(`cat > patch-registry-proxy-dns.yaml <<EOF` + "\n" + `---` + "\n" + `apiVersion: apps/v1` + "\n" + `kind: Deployment` + "\n" + `metadata:` + "\n" + `  name: registry-proxy` + "\n" + `spec:` + "\n" + `  template:` + "\n" + `    metadata:` + "\n" + `      annotations:` + "\n" + `        spiffe.io/federatesWith: nsm.cluster2,nsm.cluster3` + "\n" + `EOF`)
	r.Run(`cat > patch-registry-memory.yaml <<EOF` + "\n" + `---` + "\n" + `apiVersion: apps/v1` + "\n" + `kind: Deployment` + "\n" + `metadata:` + "\n" + `  name: registry` + "\n" + `spec:` + "\n" + `  template:` + "\n" + `    metadata:` + "\n" + `      annotations:` + "\n" + `        spiffe.io/federatesWith: nsm.cluster2,nsm.cluster3` + "\n" + `EOF`)
	r.Run(`kubectl apply -k https://github.com/networkservicemesh/deployments-k8s/examples/interdomain?ref=2cc5052cd0a4293ee06f99496be732619b471b5d`)
	...
	r.Run(`kubectl create ns nsm-system`)
	r.Run(`cat > ./registry-k8s-kustomization/patch-registry-k8s.yaml <<EOF` + "\n" + `---` + "\n" + `apiVersion: apps/v1` + "\n" + `kind: Deployment` + "\n" + `metadata:` + "\n" + `  name: registry-k8s` + "\n" + `spec:` + "\n" + `  template:` + "\n" + `    metadata:` + "\n" + `      annotations:` + "\n" + `        spiffe.io/federatesWith: nsm.cluster1,nsm.cluster2` + "\n" + `EOF`)
	r.Run(`kubectl apply -k https://github.com/networkservicemesh/deployments-k8s/apps/registry-k8s?ref=2cc5052cd0a4293ee06f99496be732619b471b5d`)
}
func (s *Suite) TestFloatingKernel2Vxlan2Kernel() {
	...
	r.Run(`export KUBECONFIG=$KUBECONFIG2`)
	r.Run(`NAMESPACE1=($(kubectl create -f https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/2cc5052cd0a4293ee06f99496be732619b471b5d/examples/interdomain/usecases/namespace.yaml)[0])` + "\n" + `NAMESPACE1=${NAMESPACE1:10}`)
	...
	r.Run(`export KUBECONFIG=$KUBECONFIG1`)
	r.Run(`NAMESPACE2=($(kubectl create -f https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/2cc5052cd0a4293ee06f99496be732619b471b5d/examples/interdomain/usecases/namespace.yaml)[0])` + "\n" + `NAMESPACE2=${NAMESPACE2:10}`)
